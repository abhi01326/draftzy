...
export const Canvas = ({ boardId }: ConvasProps) => {
  ...
  const updateSelectionMode = useMutation(
  ...
  const insertPath = useMutation((
    ...
    const layerId = nanoid();
    layers.set(layerId,  new LiveObject(penPointsToPathLayer(pencilDraft,lastUsedColor)));
    const liveLayerIds = storage.get("layerIds");
    liveLayerIds.push(layerId);
    setMyPresence({ pencilDraft: null});
    setCanvasState({mode: CanvasMode.None});
  ...
  const onPointerMove = useMutation(
    ({ setMyPresence }, e: React.PointerEvent) => {
      ...
      if (canvasState.mode === CanvasMode.Pressing) {
        ...
        updateSelectionMode(current, canvasState.origin);
    ...
    [canvasState, resizeSelectedLayer, camera, translateSelectedLayers, updateSelectionMode, startMultiSelection, continueDrawing]
  );
  ...
  const onPointerUp = useMutation(
    ...
    [camera, canvasState, insertLayer, history, insertPath, setCanvasState]
  );
  ...
  const onLayerPointerDown = useMutation(
    ...
    [setCanvasState, camera, canvasState.mode, history, unselectLayer]
  );
